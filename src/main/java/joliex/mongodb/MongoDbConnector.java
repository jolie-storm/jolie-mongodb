...
public class MongoDbConnector extends JavaService {
    ...
    public Value insertMany(Value request) throws FaultException {
        ...
        try {
            ...
            db.getCollection(collectionName, BsonDocument.class).insertMany(documents);
            for (int counterDocuments = 0; counterDocuments < request.getChildren("document").size(); counterDocuments++) {
                String str = new String(Hex.decode(documents.get(counterDocuments).get("_id").asObjectId().getValue().toHexString()), StandardCharsets.UTF_8);
            ...
            if (bsonQueryDocument.isString(keyName)) {
                ...
                if (conditionValueName.getValue().startsWith("$")) {
                    ...
                    if (request.getChildren(conditionValueVariableName).size() <= 1) {
                        ...
                        BsonArray conditionArray = new BsonArray();
                        for (int counter = 0; counter < request.getChildren(conditionValueVariableName).size(); counter++) {
                            ...
                            if (request.getChildren(conditionValueVariableName).get(counter).hasChildren("@type")) {
                                conditionArray.add(convertComplexType(request.getChildren(conditionValueVariableName).get(counter)));
                            } else {
                                conditionArray.add(convertNaturalTypeToBson(request.getChildren(conditionValueVariableName).get(counter)));
                            }

                        }
                        bsonQueryDocument.put(keyName, conditionArray);
                    }
                }

            }
            else if (bsonQueryDocument.isDocument(keyName)) {
                bsonQueryDocument.put(keyName, prepareBsonQueryData(bsonQueryDocument.getDocument(keyName), request));
            } else if (bsonQueryDocument.get(keyName).isRegularExpression()) {

                if (bsonQueryDocument.get(keyName).asRegularExpression().getPattern().startsWith("$")) {
                    String conditionValueVariableName = bsonQueryDocument.get(keyName).asRegularExpression().getPattern().substring(1);
                    BsonRegularExpression regularExpression = new BsonRegularExpression(request.getFirstChild(conditionValueVariableName).strValue(),
                            bsonQueryDocument.get(keyName).asRegularExpression().getOptions());
                    ...
                    if (bsonValue instanceof BsonString) {
                        ...
                        if (conditionValueName.getValue().startsWith("$")) {
                            ...
                            if (request.getChildren(conditionValueVariableName).size() <= 1) {
                                ...
                                bsonArray.set(bsonCounter, conditionArray);
                                bsonQueryDocument.put(keyName, conditionArray);
                            }
                        }

                    } else if (bsonValue instanceof BsonDocument) {
                        bsonArray.set(bsonCounter, prepareBsonQueryData(bsonValue.asDocument(), request));
                    }
                    bsonCounter++;
                }
                ...
                for (int counterValueVector = 0; counterValueVector < valueVector.size(); counterValueVector++) {
                    ...
                    if (valueVector.get(counterValueVector).hasChildren()) {
                        if (valueVector.get(counterValueVector).hasChildren("@type")) {
                            ...
                            if ((valueVector.size()==1) & (valueVector.get(counterValueVector).hasChildren("@asObject"))) {
                                bsonDocument.append(entry.getKey(), createDocument(valueVector.get(counterValueVector)));
                            } else {
        ...
        while (iteratorArray.hasNext()) {
            ...
            if (obj instanceof BsonObjectId) {
                BsonObjectId objId = BsonObjectId.class
                        .cast(obj);
                try {
                    String str = new String(Hex.decode(objId.getValue().toHexString()), StandardCharsets.UTF_8);
                    Value objValue = Value.create(str);
                ...
                BsonString objString = BsonString.class.cast(obj);
                Value objValue = Value.create(objString.getValue());
                vv.add(objValue);
            } else if (obj instanceof BsonInt32) {
                BsonInt32 objInt32 = BsonInt32.class.cast(obj);
                Value objValue = Value.create(objInt32.getValue());
                vv.add(objValue);
            } else if (obj instanceof BsonInt64) {
                BsonInt64 objInt64 = BsonInt64.class.cast(obj);
                Value objValue = Value.create(objInt64.getValue());
                vv.add(objValue);

            } else if (obj instanceof BsonDouble) {
                BsonDouble objDouble = BsonDouble.class.cast(obj);
                Value objValue = Value.create(objDouble.getValue());
                vv.add(objValue);
